import { Controller, Get, Post, Body, Query, HttpCode, UseGuards, Injectable, CanActivate, ExecutionContext, InternalServerErrorException, Logger } from '@nestjs/common';
import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

// MOCK AUTHENTICATION GUARD TO SECURE THE TRANSFER ENDPOINT
@Injectable()
...
export class TransactionController {
  ...
  async transferMoney(@Body() body: any) {
    ...
    const senderBalanceInCents = Math.round(Number.parseFloat(body.senderBalance) * 100);
    ...
    this.logger.log(`User ${body.senderId} transferred ${body.amount} to ${body.receiverId}. New balance: ${newBalance}`);
